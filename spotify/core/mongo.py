...
from spotify import config, logger, userbot
...
class MongoDB:
    def __init__(self):
        ...
        # Played songs cache for fast replay
        self.played_songs = {}
        self.played_songsdb = self.db.played_songs
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from spotify.helpers._admins import reload_admins
    ...
    # PLAYED SONGS CACHE METHODS
    async def add_played_song(self, chat_id: int, song_id: str, file_path: str) -> None:
        """Add a played song to cache for fast replay."""
        if chat_id not in self.played_songs:
            self.played_songs[chat_id] = {}
        self.played_songs[chat_id][song_id] = file_path
        
        # Keep only last 50 played songs per chat to avoid bloat
        if len(self.played_songs[chat_id]) > 50:
            oldest_key = next(iter(self.played_songs[chat_id]))
            del self.played_songs[chat_id][oldest_key]
        
        await self.played_songsdb.update_one(
            {"_id": chat_id},
            {"$set": {"songs": self.played_songs[chat_id]}},
            upsert=True
        )

    async def get_played_song(self, chat_id: int, song_id: str) -> str | None:
        """Get cached file path for a played song."""
        if chat_id not in self.played_songs:
            doc = await self.played_songsdb.find_one({"_id": chat_id})
            if doc:
                self.played_songs[chat_id] = doc.get("songs", {})
            else:
                self.played_songs[chat_id] = {}
        
        file_path = self.played_songs[chat_id].get(song_id)
        if file_path and os.path.exists(file_path):
            return file_path
        return None

    async def load_played_songs(self) -> None:
        """Load played songs cache from database."""
        async for doc in self.played_songsdb.find():
            chat_id = doc["_id"]
            self.played_songs[chat_id] = doc.get("songs", {})
